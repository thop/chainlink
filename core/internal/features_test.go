...
func xTestIntegration_FluxMonitor_Deviation(t *testing.T) {
	app, cleanup := cltest.NewApplicationWithKey(t,
		cltest.LenientEthMock,
		cltest.EthMockRegisterChainID,
		cltest.EthMockRegisterGetBalance,
	...
	eth.Context("Flux Monitor initializes price", func(mock *cltest.EthMock) {
		hex := hexutil.Encode(cltest.MakeRoundStateReturnData(2, true, 10000, 7, 0, availableFunds, minPayment, 1))
	...
	eth.Context("ethTx.Perform() for initial send", func(eth *cltest.EthMock) {
		...
		eth.Register("eth_getTransactionReceipt", confirmedReceipt) // confirmed for gas bumped txat
		eth.Register("eth_getBlockByNumber", &types.Header{Number: big.NewInt(1)})
...
func xTestIntegration_FluxMonitor_NewRound(t *testing.T) {
	app, cleanup := cltest.NewApplicationWithKey(t)
	defer cleanup()
	app.GetStore().Config.Set(orm.EnvVarName("MinRequiredOutgoingConfirmations"), 1)
	minPayment := app.Store.Config.MinimumContractPayment().ToInt().Uint64()
...
func TestIntegration_FluxMonitor_Deviation(t *testing.T) {
	...
	rpcClient.On("BatchCallContext", mock.Anything, mock.Anything).
		Run(func(args mock.Arguments) {
			...
			*elems[0].Result.(*types.Header) = types.Header{Number: big.NewInt(10)}
		}).
		Return(nil)
...
func TestIntegration_FluxMonitor_NewRound(t *testing.T) {
	...
	rpcClient.On("BatchCallContext", mock.Anything, mock.Anything).
		Run(func(args mock.Arguments) {
			elems := args.Get(1).([]rpc.BatchElem)
			*elems[0].Result.(*types.Header) = types.Header{Number: big.NewInt(1)}
		}).
		Return(nil).
		Maybe()
